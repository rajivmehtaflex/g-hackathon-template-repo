...
- `src/app`: App Router pages and route handlers. The main landing page is `src/app/page.tsx`; the authenticated area is `src/app/dashboard/page.tsx`; auth endpoints live under `src/app/api/auth/*`.
...
- Run `npm run test` once a suite exists; for now validate manually with `npm run dev`, `/dashboard` routing, and Supabase-backed auth/data flows.
